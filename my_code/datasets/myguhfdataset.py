...
class MyGuHFDataset(BaseDataset):
    ...
    def get_sub_data(self, 
        ...
        file_path = os.path.join(sub_info.path, 'data_' + sub_info.ID + '_64_2.mat')
        ...
        # ch_num * samples * stimulus_num * block_num
        # data = np.delete(data, 49, axis=2) # 去除50Hz刺激
        stim_freq0 = list(np.arange(1,61,1))
        idx = [stim_freq0.index(i) for i in self.stim_info['freqs']]
        num_chs,num_samples,num_stims0,num_blocks = data.shape
        num_stims1 = len(idx)

        data_pick = np.zeros((num_chs,num_samples,num_stims1,num_blocks))
        for i in range(num_stims1):
            data_pick[:,:,i,:] = data[:,:,idx[i],:]

        # block_num * stimulus_num * ch_num * sample_num
        data_pick = transpose(data_pick, (3,2,0,1))[0:6]
