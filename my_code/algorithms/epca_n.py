...
def _r_cca_canoncorr_eepca(X: ndarray,
                     Y: List[ndarray],
                     U: ndarray,
                     V: ndarray,
                     stim_freqs: List[float], 
    ...
    U : ndarray
        Spatial filter
        shape: (filterbank_num * stimulus_num * channel_num * stimulus_num * n_component)
    V : ndarray
        Weights of harmonics
        shape: (filterbank_num * stimulus_num * harmonic_num * stimulus_num * n_component)
    ...
    U = U.reshape(n_banks, n_stims, n_channels, -1)
    V = V.reshape(n_banks, n_stims, n_channels, -1)
...
class EEPCA(BaseModel):
    ...
    def fit(self,
        ...
        channel_num = template_sig[0].shape[1]
        n_component = self.n_component
        U_epca = np.zeros((filterbank_num, 1, channel_num, stimulus_num, n_component))
        possible_class = list(set(Y))
        possible_class.sort(reverse = False)
        for filterbank_idx in range(filterbank_num):
            X_train = [[X[i][filterbank_idx,:,:] for i in np.where(np.array(Y) == class_val)[0]] for class_val in possible_class]
            ...
            for stim_idx, u in enumerate(U):
                U_epca[filterbank_idx, 0, :, stim_idx, :] = u[:channel_num,:n_component]
    ...
    def predict(self,X: List[ndarray]) -> List[int]:
        ...
        if self.n_jobs is not None:
            r = Parallel(n_jobs=self.n_jobs)(delayed(partial(_r_cca_canoncorr_eepca, Y=template_sig, U=U, V=U, stim_freqs=self.stim_freqs, srate=self.srate))(X=a) for a in X)
        else:
            r = []
            for a in X:
                r.append(_r_cca_canoncorr_eepca(X=a, Y=template_sig, U=U, V=U, stim_freqs=self.stim_freqs, srate=self.srate))
