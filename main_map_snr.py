...
for i in range(sub_num):
    data.append(dataset.get_sub_data(i)[:,:,:,:int(trial_len*srate)])
...
if dataset_key == 'Benchmark':
    ...
    ch = 'OZ'
    data_length = 3
    t_s,t_e = int(0.64*250),int((0.5+0.14+data_length)*250)
    ...
    ch = 'OZ'
    data_length = 3
    t_s,t_e = int(0.14*1000),int((0.14+data_length)*1000)
    times = np.arange(0,data_length,1/1000)
if dataset_key == 'MEG':
    stim_freq = 9
    ch = 'O1'
    data_length = 3
    t_s,t_e = int(0.64*1000),int((0.5+0.14+data_length)*1000)
    times = np.arange(0,data_length,1/1000)
...
ch_idx = dataset.channels.index(ch)
data_stim = data[:,:,idx,ch_idx,:]


data_stim_pre = np.zeros(data_stim.shape)
for i in range(sub_num):
    data_stim_pre[i,:,:] = bandpass_filter(dataset, megpreprocess(dataset, data_stim[i,:,:]),7,90)

data_mean = np.mean(data_stim_pre[:,0:block_num-1],axis=(0,1))
...
for sub_idx in range(sub_num):
    for trial_idx in range(block_num):
        data_single_trial = data_stim_pre[sub_idx,trial_idx,:]
        data_stim_pre_project[sub_idx,trial_idx,:] = project(data_single_trial,int(srate/stim_freq),True,True)
...
if dataset_key == 'Benchmark':
    ...
    if savedata:
        with open(f'./result_R1/maps/EPCs/signal_Benchmark.pkl','wb') as f:
            pickle.dump([times,data_mean[t_s:t_e],data_stim_pre_project[0,0,t_s:t_e]],f)
if dataset_key == 'BETA':
    ax.plot(times,data_mean[t_s:t_e],linewidth=1.5,color=colors[0])
    ax.plot(times,data_stim_pre_project[1,0,t_s:t_e],linewidth=1.5,color=colors[1])
    ax.set_xlim(times[0],times[-1])
    ax.set_ylim(-4.5,4.5)
    ...
    ax.set_ylabel('Amplitude (uV)')
    if savedata:
        with open(f'./result_R1/maps/EPCs/signal_BETA.pkl','wb') as f:
            pickle.dump([times,data_mean[t_s:t_e],data_stim_pre_project[1,0,t_s:t_e]],f)
if dataset_key == 'eldBETA':
    ax.plot(times,data_mean[t_s:t_e],linewidth=1.5,color=colors[0])
    ax.plot(times,data_stim_pre_project[17,0,t_s:t_e],linewidth=1.5,color=colors[1])
    ax.set_xlim(times[0],times[-1])
    ax.set_ylim(-3.3,3.3)
    ax.set_xticks([0,0.5,1,1.5,2,2.5,3])
    ...
    ax.set_ylabel('Amplitude (uV)')
    if savedata:
        with open(f'./result_R1/maps/EPCs/signal_eldBETA.pkl','wb') as f:
            pickle.dump([times,data_mean[t_s:t_e],data_stim_pre_project[17,0,t_s:t_e]],f)
if dataset_key == 'GuHF':
    ax.plot(times,data_mean[t_s:t_e],linewidth=1.5,color=colors[0])
    ax.plot(times,data_stim_pre_project[2,0,t_s:t_e],linewidth=1.5,color=colors[1])
    ax.set_xlim(times[0],times[-1])
    ax.set_ylim(-3.3,3.3)
    ax.set_xticks([0,0.5,1,1.5,2,2.5,3])
    ax.set_yticks([-3,-1.5,0,1.5,3])
    ...
    ax.set_ylabel('Amplitude (uV)')
    if savedata:
        with open(f'./result_R1/maps/EPCs/signal_GuHF.pkl','wb') as f:
            pickle.dump([times,data_mean[t_s:t_e],data_stim_pre_project[2,0,t_s:t_e]],f)
elif dataset_key == 'MEG':
    ax.plot(times,data_stim_pre_project[0,0,t_s:t_e],linewidth=1.5,color=colors[1])
    ax.plot(times,data_mean[t_s:t_e],linewidth=1.5,color=colors[0])
    ax.set_xlim(times[0],times[-1])
    ...
    if savedata:
        with open(f'./result_R1/maps/EPCs/signal_MEG.pkl','wb') as f:
            pickle.dump([times,data_mean[t_s:t_e],data_stim_pre_project[0,0,t_s:t_e]],f)
...
from my_code.datasets.myguhfdataset64 import MyGuHFDataset64
from my_code.utils.guhfpreprocess import preprocess as guhfpreprocess
...
if dataset_key == 'Benchmark':
    ...
    dataset = MyGuHFDataset64(path=data_path)
...
# %%
#----------------------------------------------FBCCA map----------------------------------------
save_path = f'./result_R1/maps/maps/maps_fbcca_{dataset_key}.pkl'
...
#----------------------------------------------TRCA map----------------------------------------
save_path = f'./result_R1/maps/maps/maps_trca_{dataset_key}.pkl'
if os.path.exists(save_path):
    print(f'{save_path} exists, pass...')
else:
    from tqdm import tqdm
...
#----------------------------------------------TDCA map----------------------------------------
save_path = f'./result_R1/maps/maps/maps_tdca_{dataset_key}.pkl'
if os.path.exists(save_path):
    print(f'{save_path} exists, pass...')
else:
    from tqdm import tqdm
...
#----------------------------------------------RESS map----------------------------------------
save_path = f'./result_R1/maps/maps/maps_ress_{dataset_key}.pkl'
if os.path.exists(save_path):
    print(f'{save_path} exists, pass...')
else:
    from tqdm import tqdm
...
# ----------------------------------------------EPCA map----------------------------------------
save_path = f'./result_R1/maps/maps/maps_epca_{dataset_key}.pkl'
if os.path.exists(save_path):
    print(f'{save_path} exists, pass...')
else:
    from tqdm import tqdm
...
# 所有EEG数据集的通道顺序都一样, 因此可以使用同一个info来绘制
...
ch_names_raw_upper = [name.upper() for name in ch_names_raw]
ch_names_montage_upper = [name.upper() for name in ch_names_montage]
...
info = raw.info
# info.save('./result_R1/maps/raw_plot_info/raw_eeg-info.fif')
...
num_block = dataset.block_num
test_sig_U = np.zeros((1,len(trial_container),len(stim_freqs),num_block-1,int(tw_seq[0]*srate)))
...
for j in tqdm(range(len(trial_container))):
    ...
    # X_test: list[], 每个元素为(子带*通道*采样点)
    X_test, Y_test, ref_sig, _ = trial_container[j][1].get_data([dataset], False)

    trained_model.predict(X_test)

    Us = trained_model.model['U'] # List[num_trials], filterbank_num*stimulus_num*channel_num*n_component

    test_sig_U_ = []
    for idx,(test_todo,y) in enumerate(zip(X_test,Y_test)):
        U_todo = Us[idx][0,y,:,:]
        test_sig_U_.append(U_todo.T @ test_todo[0,:,:])

    n_class = len(np.unique(Y_test))
    for stim_idx in range(n_class):
        test_stim_idx = [i for i in range(len(Y_test)) if Y_test[i] == stim_idx]
        for idx in range(len(test_stim_idx)):
            test_sig_U[0,j,stim_idx,idx,:] = test_sig_U_[test_stim_idx[idx]]


# %%
import pickle
with open(f'./result_R1/maps/SNRs/test_sig_U_64_fbcca_{dataset_key}.pkl','wb') as f:
...
# %%
# TDCA
from my_code.algorithms.tdca import TDCA,xiang_dsp_feature,aug_2
...
for j in tqdm(range(len(trial_container))):
    ...
    n_component = trained_model.n_component

    template_sig_U_ = []
    n_class = len(np.unique(Y_train))
    for stim_idx in range(n_class):
        template_todo = trained_model.templates_[y]
        b = template_todo[:n_component, :]
        b = np.reshape(b, (-1))
        template_sig_U_.append(b)

    template_sig_U.append(np.array(template_sig_U_))

    # X_test: list[], 每个元素为(子带*通道*采样点)
    X_test, Y_test, ref_sig, _ = trial_container[j][1].get_data([dataset], False)

    X = np.array(X_test)[:,0,:,:] # TDCA不需要多子带, 取第一个子带
    X -= np.mean(X, axis=-1, keepdims=True)
    X = X.reshape((-1, *X.shape[-2:]))

    test_sig_U_ = []
    y_tdca_ = []
    for test_todo,y in zip(X,Y_test):
        Ps_todo = trained_model.Ps_[y]
        W = trained_model.W_
        M = trained_model.M_
        training=False
        ...
        a = xiang_dsp_feature(
            W,
            M,
            aug_2(test_todo, Ps_todo.shape[0], padding_len, Ps_todo, training=training),
            n_component=n_component,
        )
        a = np.reshape(a, (-1))
        
        test_sig_U_.append(a)
        y_tdca_.append(y)
    
    test_sig_U.append(np.array(test_sig_U_))
    y_tdca.append(np.array(y_tdca_))

num_block = dataset.block_num
n_class = len(np.unique(y_tdca[0]))
test_sig_U_1 = np.zeros((1,len(trial_container),len(stim_freqs),num_block-1,test_sig_U[0].shape[-1]))
template_sig_U_1 = np.zeros((1,len(trial_container),len(stim_freqs),template_sig_U[0].shape[-1]))
for j in tqdm(range(len(trial_container))):
    for stim_idx in range(n_class):
        test_stim_idx = np.where(y_tdca[j]==stim_idx)[0]
        test_sig_U_1[0,j,stim_idx,:,:] = test_sig_U[j][test_stim_idx]
        template_sig_U_1[0,j,stim_idx,:] = template_sig_U[j][stim_idx]


# %%
import pickle
with open(f'./result_R1/maps/SNRs/test_sig_U_64_tdca_{dataset_key}.pkl','wb') as f:
    pickle.dump(test_sig_U_1,f)
with open(f'./result_R1/maps/SNRs/template_sig_U_64_tdca_{dataset_key}.pkl','wb') as f:
    pickle.dump(template_sig_U_1,f)
...
# %%
from tqdm import tqdm
from SSVEPAnalysisToolbox.algorithms import TRCA,ETRCA
...
models = [
    ...
    EPCA(weights_filterbank=weights_filterbank,stim_freqs=stim_freqs,srate=srate),
    ]
...
test_sig_U = np.zeros((len(models),len(trial_container),len(stim_freqs),dataset.block_num-1,int(tw_seq[0]*srate)))
...
# %%
import pickle
with open(f'./result_R1/maps/SNRs/test_sig_U_64_trca_{dataset_key}.pkl','wb') as f:
    pickle.dump(test_sig_U[0:1],f)
with open(f'./result_R1/maps/SNRs/template_sig_U_64_trca_{dataset_key}.pkl','wb') as f:
    pickle.dump(template_sig_U[0:1],f)

with open(f'./result_R1/maps/SNRs/test_sig_U_64_ress_{dataset_key}.pkl','wb') as f:
    pickle.dump(test_sig_U[1:2],f)
with open(f'./result_R1/maps/SNRs/template_sig_U_64_ress_{dataset_key}.pkl','wb') as f:
    pickle.dump(template_sig_U[1:2],f)

with open(f'./result_R1/maps/SNRs/test_sig_U_64_epca_{dataset_key}.pkl','wb') as f:
    pickle.dump(test_sig_U[2:3],f)
with open(f'./result_R1/maps/SNRs/template_sig_U_64_epca_{dataset_key}.pkl','wb') as f:
    pickle.dump(template_sig_U[2:3],f)
...
methods = ['fbcca','trca','tdca','ress','epca']
...
if method == 'fbcca':
    import pickle
    with open(f'./result_R1/maps/SNRs/test_sig_U_64_{method}_{dataset_key}.pkl','rb') as f:
        test_sig_U = pickle.load(f)
else:
    import pickle
    with open(f'./result_R1/maps/SNRs/template_sig_U_64_{method}_{dataset_key}.pkl','rb') as f:
...
if dataset_key == 'Benchmark':
    stim_freqs = [8, 9, 10, 11, 12, 13, 14, 15, 8.2, 9.2, 10.2, 11.2, 12.2, 13.2, 14.2, 15.2, 
                  8.4, 9.4, 10.4, 11.4, 12.4, 13.4, 14.4, 15.4, 8.6, 9.6, 10.6, 11.6, 12.6, 
                  13.6, 14.6, 15.6, 8.8, 9.8, 10.8, 11.8, 12.8, 13.8, 14.8, 15.8]
    srate = 250
    block_num = 6
    trial_container_num = int(35 * 6)
elif dataset_key == 'BETA':
    stim_freqs = [8.6, 8.8, 9, 9.2, 9.4, 9.6, 9.8, 10, 10.2, 10.4, 10.6, 10.8, 11, 11.2, 11.4, 11.6, 11.8,
                  12, 12.2, 12.4, 12.6, 12.8, 13, 13.2, 13.4, 13.6, 13.8, 14, 14.2, 14.4, 14.6, 14.8, 
                  15, 15.2, 15.4, 15.6, 15.8, 8, 8.2, 8.4]
    srate = 250
    block_num = 3
    trial_container_num = int(70*4)
elif dataset_key == 'eldBETA':
    stim_freqs = [8.0, 9.5, 11, 8.5, 10, 11.5,9, 10.5, 12]
    srate = 250
    block_num = 7
    trial_container_num = int(100*7)
elif dataset_key == 'GuHF':
    stim_freqs = [10,11,12,13,14,15,20,21,22,23,24,25,40,41,42,43,44,45,55,56,57,58,59,60]
    srate = 1000
    block_num = 6
    trial_container_num = int(30*6)
elif dataset_key == 'MEG':
    stim_freqs = [9,10,11,12,13,14,15,16,17]
    srate = 1000
    block_num = 6
    trial_container_num = int(13*6)

from my_code.utils.utils import freqs_snr,nextpow2
if method != 'fbcca':
    SNR_template = np.zeros((trial_container_num,len(stim_freqs)))
    for j in range(trial_container_num):
        for stim_idx in range(len(stim_freqs)):
            s = freqs_snr(template_sig_U[0,j,stim_idx,:][:,np.newaxis].T,
                        target_fre=stim_freqs[stim_idx],
                        srate=srate,
                        Nh=5,
                        NFFT = 2 ** nextpow2(10*srate),
                        )
            SNR_template[j,stim_idx] = s

    with open(f'./result_R1/maps/SNRs/SNR_template_{method}_{dataset_key}.pkl','wb') as f:
        pickle.dump(SNR_template,f)

SNR_test = np.zeros((trial_container_num,len(stim_freqs),5))
for j in range(trial_container_num):
    for stim_idx in range(len(stim_freqs)):
        for trial_idx in range(block_num-1):
            s = freqs_snr(test_sig_U[0,j,stim_idx,trial_idx,:][:,np.newaxis].T,
                        target_fre=stim_freqs[stim_idx],
                        srate=srate,
                        Nh=5,
                        NFFT = 2 ** nextpow2(10*srate),
                        )
            SNR_test[j,stim_idx,trial_idx] = s

with open(f'./result_R1/maps/SNRs/SNR_test_{method}_{dataset_key}.pkl','wb') as f:
    pickle.dump(SNR_test,f)
...
SNR_test = np.zeros((trial_container_num,len(stim_freqs),block_num-1))
for j in range(trial_container_num):
    for stim_idx in range(len(stim_freqs)):
        for trial_idx in range(block_num-1):
            s = freqs_snr(test_sig_U[0,j,stim_idx,trial_idx,:][:,np.newaxis].T,
